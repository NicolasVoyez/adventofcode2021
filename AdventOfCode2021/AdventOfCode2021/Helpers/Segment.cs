...
namespace AdventOfCode2021.Helpers
{
    ...
    public class Segment
    {
        ...
        public IEnumerable<Point> GetIntersections(Segment s2)
        {
            if (IsParallelTo(s2))
            {
                var s2Points = new HashSet<Point>(s2.GetAllPoints());
            ...
            else
            {
                ...
                {
                    yield return new Point(iY, iX);
                }
            }
        }

        private HashSet<Point> _iterated = null;
        public HashSet<Point> GetAllPoints()
        {
            if (_iterated != null)
                return _iterated;
            _iterated = new HashSet<Point>();
            if (IsVertical)
            {
                for (int y = (FromY <= ToY) ? FromY : ToY; (FromY <= ToY) ? (y <= ToY) : (y <= FromY); y++)
                {
                    _iterated.Add(new Point( y, FromX));
                }
            }
            else
            {
                ...
                {
                    _iterated.Add(new Point(A * x + B, x));
                }
            }
            return _iterated;
        }
